...
namespace NetworkReliabilityEx1
{
    public partial class Form1 : Form
    {
        ...
        double[] TTL = new double[edgeNumber];//edge lifesycle per index 
        ...
        double[] times = { 0.0, 0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8, 0.85, 0.9, 0.95, 1.0 };
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            r = CalcProbDynamic(terminal1, terminal2, terminal3, r, m);
        ...
        private double CalcProbDynamic(int terminal1, int terminal2, int terminal3, double r, double m)
        {
            //give all edge probability 0.1, 0.2,...0.99 as instructed
            for (int z = 0; z < probNumbers; z++)
            {
                for (int i = 0; i < edgeNumber; i++)
                {
                    ///rnd.NextDouble(); //now it's random time to fail...//p[z];
                    TTL[i] = 1 - Math.Pow(e,  times[rnd.Next(0,times.Length -1)] * -1);
                }
